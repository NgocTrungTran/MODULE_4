...
count_id INT;
		SET
count_id = (SELECT COUNT(*) FROM customers c WHERE c.id = customer_id);
		IF
(count_id = 0)
		THEN
...
SET
message = 'Deposist Completed';
END IF;
END IF;
END;
//
 delimiter ;
call deposits(7, 40000000000, @mess);
SELECT @mess;
...
BEGIN
 		DECLARE
current_sender_balance DECIMAL(12,0);
        DECLARE
transaction_amount DECIMAL(12,0);
        DECLARE
fees_amount DECIMAL(12,0);
        DECLARE
count_sender_id int;
        DECLARE
count_recipient_id int;
        SET
fees_amount = transfer_amount * 0.1;
        SET
transaction_amount = transfer_amount + fees_amount;
        SET
count_sender_id = (SELECT COUNT(*) FROM customers c WHERE c.id = sender_id);
        SET
count_recipient_id = (SELECT COUNT(*) FROM customers c WHERE c.id = recipient_id);
SELECT c.balance
INTO current_sender_balance
FROM customers c
WHERE c.id = sender_id;
IF
(count_sender_id = 0 OR count_recipient_id = 0)
        THEN
...
//
 delimiter ;
CALL transfer(3, 4, 50000000, @mess);
SELECT @mess;